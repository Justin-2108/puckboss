...
function gameDate(g) {
  ...
  if (g.date && String(g.date).includes("T")) return new Date(g.date);
  if (g.date) return new Date(g.date + "T" + (g.time || "00:00"));
  return new Date("");
}

function gameTime(g) {
  if (g.time) return g.time;
  const value = g.dateTime || g.date || "";
  const match = String(value).match(/T(\d{2}:\d{2})/);
  return match ? match[1] : "";
}

function locked(g) {
  return gameDate(g).getTime() <= Date.now();
}
...
function scheduleTipRefresh() {
  ...
  tipRefreshTimer = setTimeout(async () => {
    tipRefreshTimer = null;
    if (league && document.activeElement?.matches("#tipsView input")) {
      scheduleTipRefresh();
      return;
    }
...
async function ensureMainLeague() {
  const leagueRef = doc(db, "leagues", MAIN_LEAGUE_ID);
  const leagueSnapshot = await getDoc(leagueRef);
  if (!leagueSnapshot.exists()) {
    await setDoc(leagueRef, {
      name: MAIN_LEAGUE_NAME,
      ownerUid: user.uid,
      createdAt: serverTimestamp()
    });
  }

  const memberRef = doc(db, "leagueMembers", user.uid + "_" + MAIN_LEAGUE_ID);
  ...
  league = { id: MAIN_LEAGUE_ID, name: MAIN_LEAGUE_NAME };
}
...
async function getTips(uid = user.uid) {
  ...
  await Promise.all(games.map(async game => {
    const snapshot = await getDoc(doc(db, "leagueTips", league.id, "users", uid, "games", game.id));
    if (snapshot.exists()) result[game.id] = snapshot.data();
  }));
  if (uid === user.uid && !Object.keys(result).length) {
    const old = await getDoc(doc(db, "leagueTips", league.id, "users", uid));
    if (old.exists()) return old.data().tips || {};
  }
  return result;
}
...
function gamesByDay() {
  ...
  map.forEach(list => list.sort((a, b) => gameDate(a) - gameDate(b)));
  return map;
}
...
async function renderTips() {
  if (!league) {
    $("tipsView").innerHTML = '<div class="card">Tippspiel wird geladen …</div>';
    return;
  }
...
async function renderTable() {
  ...
  $("tableView").innerHTML = '<div class="card"><h2>Rangliste</h2><table class="table"><thead><tr><th>#</th><th>PuckBoss</th><th>Punkte</th><th>Tipps</th></tr></thead><tbody>' + list.map((entry, index) => '<tr class="' + (entry.uid === user.uid ? "me" : "") + '"><td>' + (index + 1) + '</td><td>' + esc(entry.name) + '</td><td><b>' + entry.points + '</b></td><td>' + entry.count + '</td></tr>').join("") + '</tbody></table></div>';
}
...
async function renderLeague() {
  $("leagueName").textContent = league ? league.name : MAIN_LEAGUE_NAME;
  await renderTips();
  await updateStats();
}
...
onAuthStateChanged(auth, async currentUser => {
  ...
  try {
    await ensureMainLeague();
    await renderLeague();
  } catch (error) {
    console.error(error);
    $("tipsView").innerHTML = '<div class="card error">Das Tippspiel konnte nicht geladen werden.</div>';
  }
});